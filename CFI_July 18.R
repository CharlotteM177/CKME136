...
#Research Discipline - Breakdown
Table_30 <- CFI_Dataset %>%
  select(Award, RD) %>%
  group_by(RD) %>%
  summarise(Awards = n()) %>%
  arrange(desc(Awards))
Table_30_Head <- head(Table_30, 20)

Table_31 <- CFI_Dataset %>%
  select(Award, RD) %>%
  group_by(RD) %>%
  summarise(Awards = sum(Award)) %>%
  arrange(desc(Awards))
Table_31_Head <- head(Table_31, 20)

Table_32 <- head(left_join(Table_31, Table_30, by = "RD"), 10)

#Double Table
labels <- paste(format(round(Table_32$Awards.x / 1e6, 1), trim = TRUE), "M")
labels_1<- as.integer(Table_32$Awards.x)
Table_32$Funds <- format(round(labels_1 / 1e6, 1), trim = TRUE)

p1 = ggplot(data = Table_32, aes(x=RD,y=Awards.y))+
  geom_bar(stat="identity",fill="#F8766D") + 
  geom_text(aes(label=Awards.y,col="red",vjust=-0.5))+
  theme(legend.position="none",axis.title.y=element_blank(),
        axis.text.x = element_text(angle = 45, hjust = 1))
p2 = ggplot(data = Table_32,aes(x=RD,y=Awards.x))+
  geom_line(aes(group=1))+geom_text(aes(label=labels,vjust=0.5))+
  theme(legend.position="none",axis.title.y=element_blank(),
